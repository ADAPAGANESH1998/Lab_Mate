...
onFileSelected(event: any) {
  ...
  upload() {
    if (this.selectedFile) {
      this.service.uploadImage(this.selectedFile,this.userData.email).subscribe(() => {
        this.fetchImage(this.userData.email); // Refresh after upload
      });
    }
  }
  ...
  uploadForInstituteLogo() {
    if (this.selectedFileForInstituteLogo) {
      this.service.uploadImageForInstituteLogo(this.selectedFileForInstituteLogo,this.userData.email).subscribe(() => {
        this.fetchImageForInstitueLogo(this.userData.email); // Refresh after upload
      });
    }
  }
  ...
  fetchImageForInstitueLogo(email: string) {
  ...
  if (token) {
    ...
    if (email) {
      this.service.getUserByEmail(email).subscribe(
        data => {
this.userData = data,
console.log("Ganesh.........."+JSON.stringify(this.userData.email));
this.fetchImage(this.userData.email); // Load on page load
      this.fetchImageForInstitueLogo(this.userData.email);
        }
